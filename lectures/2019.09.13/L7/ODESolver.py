...
class ODESolver(ABC):
    ...
    def create_solver(cls, solver_type, f):
        SOLVER_TYPE_TO_CLASS_MAP = {
            "FE": ForwardEuler,
            "BE": BackwardEuler,
            "RK4": RungeKutta4,
        }

        if solver_type not in SOLVER_TYPE_TO_CLASS_MAP:
            raise ValueError("Bad solver type {}".format(solver_type))

        return SOLVER_TYPE_TO_CLASS_MAP[solver_type](f)
    ...
    def set_initial_condition(self, U0):
        ...
        try:
            ...
            raise IndexError(
                "Index of u out of bounds in f(u,t) func. Legal indices are %s"
                % (str(range(self.neq)))
            )
        if f0.size != self.neq:
            raise ValueError(
                "f(u,t) returns %d components, while u has %d components"
                % (f0.size, self.neq)
            )
...
class BackwardEuler(ODESolver):
    ...
    def __init__(self, f):
        ...
        try:
            ...
            raise ImportError(
                (
                    "Could not import module 'Newton'. Place Newton.py in "
                    f"this directory {os.path.dirname(os.path.abspath(__file__))}"
                )
            )
    ...
    def advance(self):
        ...
        if n >= 30:
            print(
                "Newton's failed to converge at t=%g " "(%d iterations)" % (t[k + 1], n)
            )
        return u_new
...
registered_solver_classes = [ForwardEuler, RungeKutta4, BackwardEuler]


def test_exact_numerical_solution():
    print("test_exact_numerical_solution()")
    a = 0.2
    b = 3
    ...
    for solver_class in registered_solver_classes:
        ...
        u_e = u_exact(t)
        max_error = (u_e - u).max()
        msg = "%s failed with max_error=%g" % (solver.__class__.__name__, max_error)
